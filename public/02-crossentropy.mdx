...
Alternatively, the detective could add up the total surprisal for each hypothesis (green and orange numbers in the above widget), and then compare overall _Total surprisal_  values. This corresponds to writing KL divergence like this:
...
Think of cross-entropy this way: __how surprised you are on average when seeing data from $p$ while modeling it as $q$?__ 

Explore this in the widget below. The widget shows what happens when our Bayesian detective from the previous chapter keeps flipping her coin. The red dashed line shows cross-entropy—the expected surprisal of the model $q$ as we keep flipping the coin with bias $p$. The orange line shows the entropy, which is the expected surprisal when both the model and actual bias are $p$. 
...
<HistoryExpand headline = "S = k log W">

<ImageGallery
images={[
{ src: "/fig/boltzmann.jpg", alt: "Ludwig Boltzmann" }
]}
...
{/* ,
{ src: "/fig/boltzmann2.jpg", alt: "grave" } */}

[Ludwig Boltzmann](https://en.wikipedia.org/wiki/Ludwig_Boltzmann) invented the statistical notion of entropy in the 1870s, decades before Shannon adapted it for information theory in late 1940's (more on that later). 

While physicists already knew about thermodynamic entropy (heat divided by temperature), Boltzmann's genius was realizing it was fundamentally about counting possibilities. His famous formula, carved on his gravestone in Vienna, reads: $S = k \log W$.

Here, $S$ is entropy, $k$ is a constant, and $W$ is the number of possible microstates. Sound familiar? This is exactly what we've been computing — if you have $W$ equally likely states, the entropy is $\log W$. Boltzmann was calculating <Math math="H(X) = \log n" /> for a uniform distribution over $n$ states before it was cool, just with different notation and units.

The connection between statistical physics and information theory runs deep. It lets us port physical intuitions to computer science — for instance, in machine learning we often say we're "running an LLM with a given temperature," using the physics terminology directly. We'll see more on that [later on](05-max_entropy#softmax).
</HistoryExpand>

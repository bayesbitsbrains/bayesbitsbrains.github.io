...
Searle's point: that's what computers do. Even if an AI perfectly minimizes cross-entropy and predicts every token — it's still just shuffling symbols without understanding. The person with the rulebook passes Turing's test, yet clearly isn't intelligent. So, what's left of the test, then? 

But let's examine the rulebook more carefully. The problem: it must be impossibly large! Say there are 1000 possible first questions. Your rulebook needs 1000 entries for one-question conversations, $1000^2$ for two questions, and so on. Naively, the size of the rulebook would have to grow exponentially with the length of the conversattion.<Footnote>See [this book](https://www.amazon.com/Quantum-Computing-since-Democritus-Aaronson/dp/0521199565) by Scott Aaronson for more discussion.</Footnote> So Searle's rulebook can't exist in our universe. Any real algorithm passing the Turing test must _compress_ the enormous possibility space of human conversations into something manageable.

That's exactly what ELIZA lacks. It can't compress English text the way LLMs do. Double ELIZA's rules, and our [parrot](https://en.wikipedia.org/wiki/Stochastic_parrot) might fool a few more people briefly. But, you can't build a compression algorithm out of ELIZA. It's just not good at predicting real human conversations.

Current AIs are fundamentally different. They're trained to __predict__ human text. Equivalently, they __compress__ it. This crucial difference from fakers like ELIZA forces these systems toward actual understanding. Ilya Sutskever compressed this idea perfectly:

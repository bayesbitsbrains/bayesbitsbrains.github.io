...
OK, using shorter codes for more frequent letters makes sense, but what's the best way to do this? Coding theory has the answer. Roughly speaking, it tells us to __try to give a letter with frequency $p_i$ a code-name of length $\log 1/p_i$__. 
...
The honest answer to what's the best way to compress Wikipedia is that we don't know (see the bonus section on Kolmogorov complexity), but we at least broadly understand how well different algorithms compress English text. 
...
The best algorithms in Hutter's competition have compressed by a factor of about 9x. But unfortunately, while the challenge is backed by a very deep understanding of the nature of intelligence, the precise parameters of the challenge makes it hard to win it with LLMs. The problem is that the file size is only 1GB. Remember, our LLM-based compression algorithm is only good if we don't count the size of the LLM itself. But in the competition, we naturally have to account for that. 1GB is then too small for our approach to scale. 
...
<Block>
_Predicting the next token well means that you understand the underlying reality that led to the creation of that token._
</Block>

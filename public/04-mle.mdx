...
We will solve this problem by taking $q \in Q$ that minimizes $D(p, q)$. If you remember the previous chapters, this should make plenty of sense! We will also see how a special case of this approach is equivalent to the _maximum likelihood principle_ (MLE) in statistics.
...
To find the best $\mu$, you can notice that whatever $\sigma$ is, finding the $\mu$ minimizing the right-hand side boils down to minimizing the expression $\sum_{i = 1}^{16} (x_i - \mu)^2$. It turns out this is minimized by $\mu_{best} = \frac{1}{16} \cdot \sum_{i = 1}^{16} x_i$. This formula is called [_the sample mean_](https://en.wikipedia.org/wiki/Sample_mean_and_covariance). 
...
I claim that minimizing KL to find a model $q$ of data $p$ is a pretty good approach in general. Why?  
...
Visually, I like to think of it like this. First, imagine a "potato" of all possible distributions. The empirical distribution $p$ is a single lonely point within it. Then, there's a subset of distributions $Q$ that we believe would be a good model for the data. Although KL divergence is not, technically speaking, a distance between distributions, because it's asymmetric, it's close enough to think of it as a distance. Minimizing KL is then like finding the closest point in $Q$ to $p$; or, if you want, it's like projecting $p$ onto $Q$.
...
where the sum goes over all pairs (snippet, letter) in our dataset. This is a mouthful, but we can simplify drastically. Plugging in <Math math = "p_{\textrm{emp}}(\textrm{snippet}) = 1/n"/> where $n$ is the number of snippets, we get 
...
The final formula is called the cross-entropy loss and it is as simple as it gets: To train a neural net, you look at the distribution it outputs on each snippet, take the probability <Math math = "q_{LLM}(\textrm{actual letter} | \textrm{snippet})" /> that the net assigns to the actual letter following the snippet, and compute the surprisal (i.e., $q \rightarrow \log 1/q$). You just keep summing surprisals for the snippets in your dataset. The average surprisal is the loss function that we are trying to push down during LLM training. <Footnote>In the game, I made the following simplification, both for you and AIs. Instead of asking you to predict the whole distribution for the next letter, and then computing your cross-entropy score, I computed your score as $\log k$, where $k$ is the number of guesses until the correct letter. We can view this as an 'optimistic' bound on what your true cross-entropy score would be. If you guessed the correct letter after $k$ trials, surely you would give that letter probability at most $1/k$ if I asked you for the whole distribution! </Footnote>

So, no wonder that Llama 4 has probably beaten you in the game - __being good at the game is its entire shtick__! All of the knowledge / intelligence in the [base model](https://toloka.ai/blog/base-llm-vs-instruction-tuned-llm/) happened to accumulate inside because it helps the model to be good at this game. 
...
<HistoryExpand headline = "Shannon's game">
...
[Claude Shannon](https://en.wikipedia.org/wiki/Claude_Shannon) invented information theory in 1948 (more on that in the final chapters). A few years later, he ran an experiment that pioneered the task LLMs train on today: letter prediction.

Shannon showed people a snippet of English text and had them guess the next letter. He tested this on his wife Betty and colleagues at Bell Labs. 

<ImageGallery images={[{src: "04-mle/twitter_volcano.png", alt: "volcano"}]} caption="From [xkcd](https://what-if.xkcd.com/34/)" width="60%" />

The result? Humans predicted English text with about 1 bit of uncertainty per letter — far better than the ~4.7 bits for random letters. English, it turns out, is about 75% redundant.

Shannon was measuring cross-entropy 60 years before it became the standard loss function for neural networks. Early LLMs matched Shannon's human subjects at roughly 1 bit per letter. Modern ones have surpassed us.

With his friend [Marvin Minsky](https://en.wikipedia.org/wiki/Marvin_Minsky) (famous AI pioneer), Shannon also tackled AI safety — specifically, the [off-switch problem](https://en.wikipedia.org/wiki/AI_capability_control#Difficulties). How do you ensure a machine can be turned off, even if it tries to prevent that? [Shannon's solution](https://kk.org/thetechnium/the-unspeakable/) to this problem is arguably still the state of the art, as of 2025.
</HistoryExpand>
...
<ImageGallery images={[{src: "00-introduction/questions.png", alt: "questions"}]} height={320} />
...
The problem: we have no clue what the "true" probabilities $p = (p_1, \dots, p_n)$ are! <Footnote> We could go down a philosophical rabbit hole about whether "true" probability even is a meaningful concept. But let's not. </Footnote> Hence, we can't compute the ideal KL score. All we know is what actually happened - the ground-truth row in the picture above. Technically speaking, we know an empirical distribution $\hat{p}$ where each $\hat{p}_i$ is either 0 or 1—the whole distribution is concentrated on the single list of outcomes we saw.
...
However, this only works _in the long run_. Disentangling $N$ from $N + 0.03\cdot N$ reliably would require about $1000$ to $10\,000$ questions. That's why the original riddle does not try to pretend we can actually say which expert is good. With just 5 predictions, any scoring is going to be pretty noisy. 
</Expand>
</Expand>
...
The methodology of selecting $q$ that maximizes $\prod_{i = 1}^n q(x_i)$ is called the _maximum likelihood principle_ and it's considered one of the most important cornerstones of statistics. We can now see that it's pretty much the same as our methodology of minimizing KL divergence (which is in fact a bit more general as it works also for non-uniform $p$). 

In a sense, this is not surprising at all. Let's remember how we defined KL divergence in the first chapter. It was all about the Bayesian detective trying to distinguish the truth from the imposter. But look—the detective accumulates evidence literally by _multiplying her current probability distribution by the likelihoods_. KL divergence / cross-entropy / entropy is just a useful language to talk about the accumulated evidence, since it's often easier to talk about "summing stuff" instead of "multiplying stuff". 
...
<HistoryExpand headline="Fisher information" id = "fisher">
<ImageGallery
images={[
{ src: "/fig/fisher.jpg", alt: "Ronald Fisher" },
]}
caption={"Ronald Fisher"}
/>

Ronald Fisher is often considered the most important statistician of the 20th century. He gave us maximum likelihood estimation, [sufficient statistics](https://en.wikipedia.org/wiki/Sufficient_statistic), popularized [randomized controlled trials](https://en.wikipedia.org/wiki/Randomized_controlled_trial), and much more.

One of his inventions, [Fisher information](https://en.wikipedia.org/wiki/Fisher_information), measures how much data tells us about unknown parameters. In modern terms, it's the local KL divergence between nearby distributions. Fisher was using information-theoretic ideas decades before Shannon formalized them!

Fisher would probably hate this website. Not just because he had famously strong opinions about everything, but because our opening story is that of a Bayesian detective updating her priors. That's the opposite of Fisher's vision for statistics. His viewpoint is often summarized as "let the data speak for itself"<Footnote>He probably never said it quite this way, but he'd have approved.</Footnote> — no priors, no subjective beliefs, just hard data.

This split created two camps: Fisher's [_frequentist_ statistics](https://en.wikipedia.org/wiki/Frequentist_inference) (objective, prior-free, using stuff like maximum likelihood) versus [_Bayesian_ statistics](https://en.wikipedia.org/wiki/Bayesian_statistics) (subjective, prior-dependent, using Bayes' rule all the time). 

It is an important mathematical tradition that young statisticians must choose one of these camps, work only within it, and are forbidden to speak to the other group — basically Alliance versus Horde in World of Warcraft.

<Expand advanced={true} id ="bayes-statistics" headline="More on Bayesian vs frequentist statistics">
Here's how Bayesian statistics works: The statistician (detective) starts with prior probabilities $p_1, \dots, p_n$ over competing hypotheses. She gathers evidence and computes each hypothesis's likelihood $\ell_i$ — the probability of seeing that evidence if hypothesis $i$ were true. Then she multiplies to get $p_1\ell_1, \dots, p_n \ell_n$, normalizes them to sum to one, and voilà: posterior distribution $p_1', \dots, p_n'$.

Bayesian statistics is really as simple as that -- just keep applying Bayes' rule; all the rest is commentary. The framework is extremely elegant, but wouldn't it be nice if we could: 1) get a single answer instead of working with distributions, and 2) skip the priors entirely?

With enough data, both become possible.

For the first problem, just pick the hypothesis with the highest posterior probability $p'_i$. Once you have enough evidence, one hypothesis usually dominates anyway. This is called [maximum a posteriori estimation](https://en.wikipedia.org/wiki/Maximum_a_posteriori_estimation) — still within Bayesian camp, but simpler.

For the second problem, notice that priors matter less with more data. Consider a coin that's either fair (50/50) or biased (25/75). The KL divergences between the two distributions are both about 0.2 bits per flip. So after $n$ flips, we expect roughly $0.2n$ bits of evidence for the truth.

Starting with prior odds like 1:2 only shifts things by 1 bit from 1:1 — equivalent to ~5 coin flips. With hundreds of flips, who cares about 5? In that regime, we can ignore priors and just pick the hypothesis with the highest likelihood $\ell_i$ — the maximum likelihood approach. Since that works without reliance on priors, we are now in the realm of frequentist statistics. 

I sometimes view the maximum likelihood principle as "poor man's Bayesian statistics". It shows how one can come up with it and explains the flip side — if you want to estimate variance from $n=3$ samples, maybe don't obsess over whether to divide by 2, 3, or 4 like in [our riddle](00-riddles/statistics). Your "objective" approach is already on shaky ground with so little data.

I still haven't figured out how to think of the Bayesian vs frequentist split. On one hand, many modern applications happily mix both approaches, so perhaps they're just two sides of the same coin. Or maybe not. Frequentist statistics really does think differently — it's about __worst-case guarantees__ that always hold, while Bayesian statistics is about __beliefs__ and how to update them.

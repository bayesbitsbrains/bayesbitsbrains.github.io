...
<Math id="cross-loss-kl" displayMode={true} math = "q_{\textrm{new}} = \argmin_{q\in Q} D(p, q)  = \argmin_{q\in Q} H(p, q) =  \argmin_{q\in Q} \sum_x p(x) \cdot \log \frac{1}{q(x)}" />
...
[Claude Shannon](https://en.wikipedia.org/wiki/Claude_Shannon) invented information theory in 1948 (more on that in the final chapters). A few years later, he ran an experiment that ended up as the task LLMs train on today: letter prediction.
...
The result? Humans predicted English text with about 1 bit of uncertainty per letter. We have, it seems, about the same intelligence as GPT-3 or GPT-4. 
...
It is an important mathematical tradition that young statisticians must choose one of these camps early on, work only within it, and are forbidden to speak to the other group — basically Alliance versus Horde in World of Warcraft.

...
We will first understand what ideas KL divergence, cross-entropy and entropy stand for. Those are the secret weapons that underlie a good bit of machine learning. We will then develop maximum likelihood and maximum entropy principles and see how to use them to set up loss functions. We will see what all this has to do with coding and gain intuition behind large language models (LLMs). 
